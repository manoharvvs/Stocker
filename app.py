...
# SNS Topic ARNS
USER_ACCOUNT_TOPIC_ARN = "Your ARN for User Account Topic"
TRANSACTION_TOPIC_ARN = "Your ARN for Transaction Topic"
...
def login():
          if request.method == 'POST':
                    ...
                    print("Trying to login with: {email} ({role})")
...
def delete_trader(trader_id):
    if 'email' not in session or session.get('role') != 'admin':
        flash("Access denied. Admins only.", "danger")
        return redirect(url_for('login'))  # <-- Missing return
...
def service05():
    ...
    try:
        ...
        print("Error calculating portfolio value: {str(e))")
    ...
    for transaction in transactions:
        if 'transaction_date' in transaction and transaction['transaction_date']:
            try:
                #Convert 150 string to datetime object if it's not already
